namespace Shapes2D
{
	...
	public class Shape : MonoBehaviour, IMaterialModifier
	{
		...
		void Start()
		{
			...
			Configure();
			Vector3[] verts = { new Vector3(-1, 1, 0), new Vector3(-1, -1, 0), new Vector3(1, -1, 0), new Vector3(1, 1, 0) };
			SetPolygonWorldVertices(verts);
		}
		...
		{
			string dname = "Assets/Resources/Sprites";
			string fname = dname + "/" + this.name + ".png";
			string rname = "Sprites/" + this.name;
			...
			Vector2 pivot = RenderToTexture2D(fname, this, pixelsPerUnit: pixelsPerUnit);
			...
			Undo.RecordObjects(this.GetUndoObjects().ToArray(),
					"Convert to Sprite");

			this.SetAsSprite(sprite, GetDefaultSpriteMaterial());
